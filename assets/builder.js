FormCraftApp.controller('MailChimpController', function($scope, $http) {
	$scope.addMap = function(){
		...
		$scope.$parent.Addons.MailChimp.Map.push({
			...
			'listName': jQuery('#mc-map .select-list option:selected').text(),
			'columnID': $scope.SelectedColumn,
			'columnName': jQuery('#mc-map .select-column option:selected').text(),
			'formField': jQuery('#mc-map .select-field').val()
		});
	}
